"""aiohttp integration helpers.

This module stays dependency-light by using protocols instead of importing
aiohttp directly. It provides async wrappers that route session requests
...
class AsyncClientSessionLike(Protocol[ResponseT_co]):
    def request(self, method: str, url: object, **kwargs: Any) -> Awaitable[ResponseT_co]: ...
...
async def arequest_with_retry(
    session: AsyncClientSessionLike[ResponseT],
    ...
    **request_kwargs: Any,
    ...
    """
    Execute `session.request(...)` wrapped by an async policy call.
    """
...
class AsyncRetryingAiohttpSession(Generic[ResponseT]):
    """
    Thin wrapper that applies an async policy to every session request call.
    """
    ...
    async def __aenter__(self) -> "AsyncRetryingAiohttpSession[ResponseT]":
        enter = getattr(self.session, "__aenter__", None)
        if callable(enter):
            await enter()
        return self
    ...
    async def __aexit__(
        ...
        exit_method = getattr(self.session, "__aexit__", None)
        if callable(exit_method):
            result = await exit_method(exc_type, exc, tb)
